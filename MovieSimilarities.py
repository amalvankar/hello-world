# To run locally:
# !python MovieSimilarities.py --items=ml-100k/movies.csv ml-100k/data.csv > sims.txt

# All the files should be present in the home directory of your computer.
# For eg Home directory for me is as follows : C:\Users\Ashutosh
...
class MovieSimilarities(MRJob):
    ...
    def load_movies_movielens(self):
    ...
    def steps(self):
        return [
            MRStep(mapper=self.mapper_parse_input_first,
                    reducer=self.reducer_ratings_given_by_user),
            MRStep(mapper=self.mapper_item_pairs,
                    reducer=self.reducer_calculate_similarityscore),
            MRStep(mapper=self.mapper_sortby_similarityscore,
                    mapper_init=self.load_movies_movielens,
                    reducer=self.reducer_output_similarities)]

    def mapper_parse_input_first(self, key, line):
        # Outputs userID => (movieID, rating)
        (userID, movieID, rating, timestamp) = line.split(',')
        yield  userID, (movieID, float(rating))

    def reducer_ratings_given_by_user(self, user_id, itemRatings):
    ...
    def mapper_item_pairs(self, user_id, itemRatings):
        # Finding every pair of movies each user has seen, and emit it
        # each pair with its related ratings
    ...
    def reducer_calculate_similarityscore(self, moviePair, ratingPairs):
        # Compute the similarity score between the ratings vectors
        # for each movie pair viewed by more than one person
    ...
    def mapper_sortby_similarityscore(self, moviePair, scores):
...
#Based on Taming Big Data With Hadoop and Map Reduce by Frank Kane